 ...
 * # 将WAL和MemTable的写入pipeline化
     当一个线程写完毕WAL之后 等待的其他的write则会开始继续写入WAL
     当前线程将会继续写入MemTable
     将不同的Writer的写入WAL和写入MemTable并发执行了
 */
...
/** comment by hy 2020-06-29
 * # 由leader线程通知所有follower线程并发写入memtable
     否则，由leader线程串行将所有follower线程的操作写入memtable中
 */
