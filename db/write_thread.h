...
/** comment by hy 2020-06-27
 * # 负责串行地将所有follower写入memtable的leader
 */
...
/** comment by hy 2020-06-27
 * # Write和WriteGroup通过WriteThread::EnterAsBatchGroupLeader关联
     当写线程要提交事务时会将自己对应的Write实例添加到Write链表的尾部
     此时存在一种特殊情况，即当前待提交的线程是加入Write链表的第一个线程
 ...
 * # 一个WriteThread::Writer代表一个写线程
     ...
     多个线程同时写，就会有多个线程同时走到该函数中
     生成多个一个WriteThread::Writer,
...
/** comment by hy 2020-06-27
 * # 一个链表的结构，
     待提交的事务可以通过JoinBatchGroup(&w)函数
     将本WriteBatch对应的Write实例加到Write链表中
     自然地，Write链表中的一个元素代表着一个待提交的写线程
     写到WAL文件中的内容有先后顺序，这里也只需要按照链表中的先后顺序写入即可
     多个Write对象的实例同样合并成一个写WAL操作，由一个线程负责进行fsync即可
     WriteThread::CreateMissingNewerLinks
     指向最老的writer WriteThread::LinkOne
 */
